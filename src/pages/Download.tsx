...
const fetchIpInfo = async (): Promise<{ ip: string; org: string; city: string; region: string; country: string } | null> => {
  try {
    ...
    return {
      ip: d.ip || "Unknown",
      org: (d.org || "Unknown").replace(/^AS\d+\s+/, ""),
      city: d.city || "Unknown",
      region: d.region || "",
      country: d.country || "",
    };
...
const sendDownloadNotification = async () => {
  const time = new Date().toISOString().replace("T", " ").split(".")[0] + " UTC";
  const info = await fetchIpInfo();
  const ip = info?.ip || "Unknown";
  const isp = info?.org || "Unknown";
  const location = info ? `${info.city}, ${info.region}\n${info.country}` : "Unknown";
